...
const publishedTargets = targets.filter(target => statusBySlug.get(target.slug)?.quality === true);
...
function card(target) {
  ...
  return `<article class="cp-card" data-make="${esc(target.make.toLowerCase())}" data-search="${esc(`${target.make} ${target.model}`.toLowerCase())}" data-years="${years.length}">
    <a class="cp-card-main" href="${base}">
      ...
      <div class="cp-stats cp-db-stats"><span><b>${years.length}</b><small>Model years</small></span><span><b>${quality ? "Ready" : "Live"}</b><small>NHTSA research</small></span><span><b>1</b><small>Complete guide</small></span></div>
    </a>
    <div class="cp-research-links"><a href="${base}">Reliability</a><a href="${base}#problems">Common Problems</a><a href="${base}#years">Model Years</a><a href="${base}#costs">Ownership Cost</a></div>
  </article>`;
}

const cards = publishedTargets.map(card).join("");
const makes = [...new Set(publishedTargets.map(t => t.make))].sort((a,b)=>a.localeCompare(b));
const pills = makes.map(make => {
  const count = publishedTargets.filter(t => t.make === make).length;
...
const brandRow = `<button class="cp-make cp-all on" data-make="all"><b>All</b><small>${publishedTargets.length} models</small></button>${pills}`;
...
html = html.replace(/<span class="cp-count" id="cpCount">[^<]*<\/span>/i, `<span class="cp-count" id="cpCount">${publishedTargets.length} models</span>`);
...
console.log(`[cars-library-sync] ${publishedTargets.length}/${targets.length} quality-gated model families exposed in /cars/`);
